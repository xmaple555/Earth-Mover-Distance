...
public class emd_class {
	...
	double init(signature_t Signature1, signature_t Signature2) {
		...
		_C = new double[MAX_SIG_SIZE + 1][MAX_SIG_SIZE + 1];

		_IsX = new int[MAX_SIG_SIZE + 1][MAX_SIG_SIZE + 1];
		_X = new node2_t[MAX_SIG_SIZE * 2];
		...
		if (Boolean.valueOf(Math.abs(diff) >= EPSILON * sSum)) // 有小數點誤差
		{
			if (diff < 0.0) {
				for (j = 0; j < _n2; j++)
					_C[_n1][j] = 0;
...
class feature_t implements Cloneable {
	double x, y, z;
	...
	public feature_t(int a, int b, int c) {
		x = a;
		y = b;
		z = c;
